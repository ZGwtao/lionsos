...
void tsldr_caputil_load_cap_from_backup_cnode(seL4_Word dest_idx, seL4_Word src_idx)
{
    seL4_Word err = seL4_CNode_Move(
        ...
        CNODE_BACKGROUND_CAP, src_idx, PD_CAP_BITS
    );
    if (err != seL4_NoError) {
...
{
    seL4_Word err = seL4_CNode_Move(
        CNODE_BACKGROUND_CAP, dest_idx, PD_CAP_BITS,
...
{
    // TODO: add log here
    tsldr_caputil_load_cap_from_backup_cnode(CNODE_VSPACE_CAP, BACKGROUND_VSPACE_CAP);
}
...
{
    // TODO: add log here
    tsldr_caputil_store_cap_to_backup_cnode(BACKGROUND_VSPACE_CAP, CNODE_VSPACE_CAP);
}
...
#endif
    seL4_Word target_idx = CNODE_BASE_MAPPING_CAP;
    seL4_Word backup_idx = BACKGROUND_MAPPING_BASE_CAP + page_idx;
...
void tsldr_caputil_revoke_irq_cap(seL4_Word irq_idx)
{
    ...
    tsldr_caputil_delete_cap(CNODE_IRQ_BASE_CAP + irq_idx);
}
...
void tsldr_caputil_revoke_ppc_cap(seL4_Word ppc_idx)
{
    ...
    tsldr_caputil_delete_cap(CNODE_PPC_BASE_CAP + ppc_idx);
}
...
void tsldr_caputil_revoke_notification_cap(seL4_Word ntfn_idx)
{
    ...
    tsldr_caputil_delete_cap(CNODE_NTFN_BASE_CAP + ntfn_idx);
}
...
void tsldr_caputil_restore_irq_cap(seL4_Word irq_idx)
{
    ...
    tsldr_caputil_copy_cap_from_backup_cnode(CNODE_IRQ_BASE_CAP + irq_idx, BACKGROUND_IRQ_BASE_CAP + irq_idx, seL4_AllRights);
}
...
void tsldr_caputil_restore_ppc_cap(seL4_Word ppc_idx)
{
    ...
    tsldr_caputil_copy_cap_from_backup_cnode(CNODE_PPC_BASE_CAP + ppc_idx, BACKGROUND_PPC_BASE_CAP + ppc_idx, seL4_AllRights);
}
...
void tsldr_caputil_restore_notification_cap(seL4_Word ntfn_idx)
{
    ...
    tsldr_caputil_copy_cap_from_backup_cnode(CNODE_NTFN_BASE_CAP + ntfn_idx, BACKGROUND_NTFN_BASE_CAP + ntfn_idx, seL4_AllRights);
}
...
void tsldr_caputil_pd_page_map(seL4_Word page_idx, uintptr_t vaddr, seL4_CapRights_t rights, seL4_Word attrs)
{
    ...
    err = seL4_X86_Page_Map(page_idx, CNODE_VSPACE_CAP, vaddr, rights, attrs);
#elif defined(CONFIG_ARCH_AARCH64)
    err = seL4_ARM_Page_Map(page_idx, CNODE_VSPACE_CAP, vaddr, rights, attrs);
#else
