...
#endif
    seL4_Word size_sh = 0;
    seL4_Word target_sh = tsldr_miscutil_fetch_elf_section_with_vaddr(elf_base, vaddr, &size_sh);
    if (!target_sh) {
...
void monitor_patch_payload_with_ossvc__worker_func(int cid, protocon_svc_t *svc, protocon_svc_req_t *req, tsldr_acrtreq_t *req_acrt, uintptr_t payload_base)
{
    if (!svc->svc_init) {
        return;
    }
...
void monitor_patch_payload_with_ossvc_info(int cid, protocon_svc_req_t *req, uintptr_t payload_base, uintptr_t monitor_svcdb_base)
{
    ...
    for (int i = 0; i < svcdb->svc_num; ++i) {
        ...
        monitor_patch_payload_with_ossvc__worker_func(cid, &curr_svc[i], req, &req_acrt, payload_base);
    }
